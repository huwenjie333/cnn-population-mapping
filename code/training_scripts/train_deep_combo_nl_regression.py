...
LOG_DIR = '/home/timhu/dfd-pop/logs/regression_l8s1nl_test'
...
#             outputs = tf.squeeze(outputs) # change shape from (B,1) to (B,), same as label input
    ...
    if MODEL == 'vgg':
        model_variables = slim.get_variables_to_restore(exclude=[
            'vgg_16/fc5','vgg_16/fc7/dim_reduce','vgg_16/fc8','vgg_16/l8', 'vgg_16/s1']) #, 'vgg_16/batch_norm']) 
    if MODEL == 'resnet':
...
#     print('variables to update in traing: ', vars_update)
    ...
    for step in xrange(MAX_STEPS):
        ...
        images_out, nl_out, labels_out = sess.run([train_images_batch, train_nl_batch, train_labels_batch])
...
#         print('NL values:', nl_out[0])
        ...
        if step % 50 == 0 or (step + 1) == MAX_STEPS: # calculate and print validation loss every 50 batches 
            images_out, nl_out, labels_out = sess.run([val_images_batch, val_nl_batch, val_labels_batch])
            feed_dict = {images_l8_placeholder: images_out[:,:,:,:3],
                         images_s1_placeholder: images_out[:,:,:,3:],
                         vectors_nl_placeholder: nl_out,
                         labels_placeholder: labels_out}
